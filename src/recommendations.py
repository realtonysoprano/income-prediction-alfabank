...
def _get_base_recommendations(income: float, segment: str = None) -> List[str]:
    ...
    if segment and segment in PRODUCTS_BY_SEGMENT:
        return PRODUCTS_BY_SEGMENT[segment][:]

    if income >= INCOME_THRESHOLDS['high']:
        return PRODUCTS_BY_SEGMENT['stable_high_income'][:]
    elif income >= INCOME_THRESHOLDS['median']:
        return PRODUCTS_BY_SEGMENT['medium_risky'][:]
    return PRODUCTS_BY_SEGMENT['low_stable'][:]
...
def _apply_shap_boosters(base_recs: List[str], shap_explanation: list) -> List[str]:
    ...
    for item in shap_explanation[:3]:
        ...
        if feat_norm in SHAP_BOOSTERS:
            recs.append(SHAP_BOOSTERS[feat_norm])
...
def _apply_segment_rules(recommendations: List[str], segment: str) -> List[str]:
    ...
    exclusions_norm = [ex.lower() for ex in exclusions]

    filtered = [
        r for r in recommendations
        if not any(ex in r.lower() for ex in exclusions_norm)
    ]
...
def get_recommendations_debug(client_id: int, income: float,
        ...
        shap_feats = (
            [item.get('feature') for item in shap_explanation[:3]]
        ...
        return {
            ...
            "ml_segment": segment,
            "shap_top3": shap_feats,
            "final_recommendations": get_recommendations(client_id, income, shap_explanation, segment),
